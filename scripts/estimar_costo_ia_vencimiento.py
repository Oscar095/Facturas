"""Estima el costo mensual de usar IA para la fecha de vencimiento.

Mide tokens REALES con /v1/messages/count_tokens (gratis, no genera respuesta)
sobre una muestra de las facturas que hoy quedarían sin vencimiento, y
extrapola al ritmo mensual observado en la BD.

Uso: .venv/Scripts/python.exe scripts/estimar_costo_ia_vencimiento.py [muestra]
"""
import sys
...
from app.services import vencimiento_ia  # noqa: E402
...
TOKENS_SALIDA = 12                  # max_tokens=20; una fecha son ~10 tokens
...
def contar(pdf, texto, fecha):
    """Arma el MISMO mensaje que enviaría vencimiento_ia y cuenta sus tokens."""
    import base64

    texto = (texto or "").strip()
    if len(texto) >= vencimiento_ia._MIN_TEXTO_UTIL:
        contenido = [{"type": "text", "text":
                      f'{vencimiento_ia._INSTRUCCION}\n\nTexto de la factura:\n'
                      f'"""{texto[:vencimiento_ia._MAX_TEXTO]}"""'}]
    else:
        contenido = [
            {"type": "document", "source": {"type": "base64",
             "media_type": "application/pdf",
             "data": base64.b64encode(vencimiento_ia._primera_pagina(pdf)).decode()}},
            {"type": "text", "text": vencimiento_ia._INSTRUCCION},
        ]
    if fecha:
        contenido[-1]["text"] += f"\nLa factura fue emitida el {fecha.date().isoformat()}."
    r = cliente.messages.count_tokens(
        model=vencimiento_ia.MODELO,
        system="Extraes un solo dato de facturas. Respondes SOLO la fecha o NULL.",
...
print(f"Facturas que hoy quedan sin vencimiento: {len(pendientes)}")
...
for etiqueta, grupo, necesita_pdf in (("texto", con_texto, False),
    ...
    for f in grupo[:MUESTRA]:
        ...
        try:
            t = contar(pdf, f.texto_pdf, f.fecha_emision)
            tokens.append(t)
...
# proporción de facturas que llegan a la IA (las que el regex no resuelve)
p_texto = len(con_texto) / total
p_vision = len(escaneadas) / total

n_texto = por_mes * p_texto
n_vision = por_mes * p_vision
